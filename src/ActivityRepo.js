...
class ActivityRepo {
  ...
  findAverageAmtStairsClimbed(dateOf) {
    let usersActiveOnDate = this.activityData.filter(
      user => user.date === dateOf
    );
    let allStairs = usersActiveOnDate.map(user => user.flightsOfStairs);
    let averagePerUserClimbed = allStairs.reduce(
      (totalStairs, stairsPerUser) => (totalStairs += stairsPerUser)
    );
  ...
  findAverageAmtStepsTaken(dateOf) {
    ...
    let averagePerUserSteps = allSteps.reduce(
      (totalSteps, stepsPerUser) => (totalSteps += stepsPerUser)
    );
  ...
  findAverageMinActive(dateOf) {
    ...
    let averageActiveMinsPerUser = allMins.reduce(
      (totalMins, minsPerUser) => (totalMins += minsPerUser)
    );
