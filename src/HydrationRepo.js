...
class HydrationRepo {
  ...
  weeklyConsumptionAverage(specificDate) {
    let newDateIndex = this.hydrationUserData.findIndex(
      user => user.date === specificDate
    );
    let slicedDates = this.hydrationUserData.slice(
      newDateIndex - 6,
      newDateIndex + 1
    );
    let weeklyOzs = slicedDates.map(user => user.numOunces);
    return Math.floor(
      weeklyOzs.reduce((totalOzs, dailyOz) => {
