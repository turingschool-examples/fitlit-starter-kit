class Sleep {
  ...
  findSleepTimeByDate(date) {
    let userSleepData = this.usersSleepData.filter(sleepData =>
      sleepData.userID === this.userID)
  ...
  findAverageSleepQualityByWeek(dateRange) {
    ...
    dateRange.forEach(date => {
      userSleepData.map(data => {
        if (date === data.date) {
          userWeekSleepQuality.push({date: data.date,
    ...
    usersDataID.forEach(userID => {
      usersAverageSleepQualityData.push(
        {name: this.userInfoName(userID, userDatas),
          usersAverageSleepQuality:
          this.userSleepQualityAverage(userID, dateRange)})
    });

    let usersAverageGreaterThanThree = usersAverageSleepQualityData
      .filter(data => data.usersAverageSleepQuality > 3);
  ...
  userSleepQualityAverage(id, dateRange) {
    ...
    let sleepData = this.usersSleepData
      .filter(sleepData => sleepData.userID === id);
    ...
    let averageSleepQuality = (userSleepQualityTotal / dateRange.length)
      .toFixed(1);
  ...
  findUsersMostSleepTimeByDay(date) {
    ...
    let longestSleeper = sleepDate.sort((a, b) =>
