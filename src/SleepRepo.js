...
class SleepRepo {
  ...
  findAboveAverageSleepers(dateOf) {
    ...
    return uniqueUsers.reduce((allIds, curId) => {
      ...
      let overallSleepQuality =
        week.reduce((overallQuality, dailyQuality) => {
  ...
  findUserWithMostHours(dateOf) {
    ...
    let longestSleeper = userHours.filter(
      day => day.hoursSlept === userHours[0].hoursSlept
    );
  ...
  findWorstSleeper(dateOf) {
    ...
    let worstSleeper = userHours.filter(
      day => day.hoursSlept === userHours[0].hoursSlept
    );
