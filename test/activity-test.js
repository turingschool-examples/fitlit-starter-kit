...
describe('Activity', function() {
    ...
    it.only('should calculate the miles a user has walked for a day', function() {
    ...
    it.only('should return a users active minutes for a specific date', function() {
        const userRepo = new UserRepository(userSampleData) 
        const user = userRepo.returnUserData(1);
        const activity = new Activity(activitySampleData)
        expect(activity.getMinutesActive(1, '2019/06/16', 'minutesActive')).to.equal(175)
    })
    it.only('should return a users average minutes active for a week', function() {
        const userRepo = new UserRepository(userSampleData) 
        const user = userRepo.returnUserData(1);
        const activity = new Activity(activitySampleData)
        expect(activity.getAverageActivityForWeek(1, '2019/06/25', 'minutesActive')).to.equal(156.71)
    })
    it.only('should tell whether a user has met their step goal for a certain day', function() {
        const userRepo = new UserRepository(userSampleData) 
        const user = userRepo.returnUserData(1);
        const activity = new Activity(activitySampleData)
        expect(activity.returnStepGoalMet(1, '2019/06/25')).to.equal(false)
    })
    it.only('should give the days the user has exceeded their step goal', function() {
        const userRepo = new UserRepository(userSampleData) 
        const user = userRepo.returnUserData(1);
        const activity = new Activity(activitySampleData)
    ...
    it.only('should return when the user climbed the most stairs', function() {
        const userRepo = new UserRepository(userSampleData) 
        const user = userRepo.returnUserData(1);
        const activity = new Activity(activitySampleData)
    ...
    it.only('should return the steps a user has walked for today', function () {
        const userRepo = new UserRepository(userSampleData)
        const user = userRepo.returnUserData(1);
        const activity = new Activity(activitySampleData)
    ...
    it.only('should return when a user has an increasing step trend for 3 days', function() {
        const userRepo = new UserRepository(userSampleData)
        const user = userRepo.returnUserData(1);
        const activity = new Activity(activitySampleData)
