...
describe('Sleep', function() {
  ...
  it('should return average hours slept per day for user for all time',
    function() {
      expect(sleep.calculateAverageSleepTimeOverall()).to.equal(5.1);
    });

  it('should return average sleep quality per day for a user for all time',
    function() {
  ...
  it('should return daily sleep hours for a week', function() {
    ...
    expect(sleep.findAverageSleepHourByWeek(userDateRange)).to.deep
      .equal([{date: "2019/06/16", sleepHours: 4.1},
        {date: "2019/06/17", sleepHours: 8},
        {date: "2019/06/18", sleepHours: 10.4},
        {date: "2019/06/19", sleepHours: 10.7},
        {date: "2019/06/20", sleepHours: 9.3},
        {date: "2019/06/21", sleepHours: 7.8},
        {date: "2019/06/22", sleepHours: 7}]);
  });

  it('should return daily sleep quality for a week', function() {
    sleep = new Sleep(usersRepository1, sleepData1)

    expect(sleep.findAverageSleepQualityByWeek(userDateRange)).to.deep
      .equal([{date: "2019/06/16", sleepQuality: 3.8},
  ...
  it('should find all users who average sleep quality > 3 for a week',
    function() {
      sleep = new Sleep(usersRepository1, sleepData1);

      expect(sleep.findAllUserSleepQualityOverThree(userData, userDateRange))
        .to.deep.equal([{name: 'Jarvis Considine',
          usersAverageSleepQuality: '3.3'}]);
    });

  it('should find the user with the highest sleep time on a given date',
    function() {
      expect(sleep.findUsersMostSleepTimeByDay('2019/06/15')).to.equal(2);
    });

  it('should find the user with the highest sleep average overall',
    function() {
